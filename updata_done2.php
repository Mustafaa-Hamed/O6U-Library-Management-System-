...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
    ...
    foreach ($objPHPExcel->getActiveSheet()->getRowIterator() as $row) {
      ...
      $sql_select = "SELECT * FROM staffs WHERE staff_id = '" . $data[0] . "'";
      $result = $conn->query($sql_select);

      if ($result->num_rows > 0) {
        //the record already exists, update it
        $sql_update = "UPDATE staffs SET name = '" . $data[1] . "', college = '" . $data[2] . "', staff_position = '" . $data[3] . "', SSN = '" . $data[4] . "' WHERE staff_id = '" . $data[0] . "'";
        ...
        $sql_insert = "INSERT INTO staffs (staff_id, name, college, staff_position, SSN)
